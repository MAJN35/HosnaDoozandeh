...
export const ExperienceTimeline: React.FC<ExperienceTimelineProps> = ({ lang }) => {
  ...
  return (
    <section id="journey" className="py-16 sm:py-24 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        ...
        <div className="space-y-10 sm:space-y-12">
          {experiences.map((exp, index) => {
            ...
            return (
              ...
              >
                {/* Neumorphic Node */}
                <div className="absolute right-6 rtl:right-6 ltr:left-6 md:right-1/2 md:rtl:right-1/2 md:ltr:left-1/2 -translate-x-1/2 z-10 w-9 h-9 rounded-full neu-circle-node flex items-center justify-center">
                  <div
                    className={`w-3.5 h-3.5 rounded-full ${
                      exp.isCurrent ? 'bg-[#3D5A80]' : 'bg-[#8FA8C8]'
                    }`}
                  />
                </div>
                ...
                >
                  <div className="neu-card p-6 sm:p-8">
                    ...
                    {/* Institution / مؤسسه */}
                    <div className="flex items-center gap-1.5 text-xs font-normal text-[#8FA8C8] mb-3">
